...
export function SaveStatus({ state, onReload }: { state: SaveState; onReload: () => void }) {
  ...
  if (state === "version_conflict") {
    return (
      <div className="flex items-start gap-2 rounded-md border border-border-default bg-surface-raised px-3 py-2">
  ...
  return <span className="text-body-small text-text-tertiary">{quiet}</span>
}
