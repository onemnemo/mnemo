...
export function NewTile({ label, onClick }: { label: string; onClick: () => void }) {
  return (
    <button
      type="button"
      onClick={onClick}
      className="group flex h-full min-h-[186px] w-full flex-col items-center justify-center gap-2.5 rounded-xl border border-dashed border-line text-ink-2 transition-colors hover:border-ink-3 hover:bg-frame-hover"
    >
      <span className="grid size-[34px] place-items-center rounded-lg bg-canvas-sunken text-ink-icon transition-colors group-hover:bg-canvas">
