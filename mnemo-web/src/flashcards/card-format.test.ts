...
describe("parseCardText markers", () => {
  ...
  it("reads the adjacent markers produced by combining bold and italic", () => {
    expect(one("***both***")).toEqual([{ bold: [{ italic: ["both"] }] }])
  })

  it("reads the repeated markers produced by pressing bold twice", () => {
    expect(one("****twice****")).toEqual([{ bold: [{ bold: ["twice"] }] }])
  })
