...
describe("CardTypeOverlay unlisted card guard", () => {
  it("asks before an ordinary edit would sweep a card under a missing layout", async () => {
    mocks.previewCardTypeSave.mockResolvedValue({ removedCardCount: 2, affectedFactCount: 2 })
    open()
    await settle()
  ...
  it("saves an ordinary edit when preflight finds no card loss", async () => {
    open()
    await settle()
