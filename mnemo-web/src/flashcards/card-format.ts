...
function tokenize(units: Unit[]): Token[] {
  ...
  while (at < units.length) {
    if (
      ...
      const after = units[at + 3]
      const run = delimiter("***", "bold", before, after)
      // The toolbar can place bold around italic or italic around bold. A closing run reverses
      // the opening order so both paths produce properly nested delimiters.
      const order: [string, CardMark][] = run.closes && !run.opens
        ? [["*", "italic"], ["**", "bold"]]
        : [["**", "bold"], ["*", "italic"]]
      for (const [marker, mark] of order)
        tokens.push({ ...run, marker, mark })
      at += 3
      continue
    }
    ...
    if (found) {
      ...
      tokens.push(delimiter(marker, mark, before, after))
      at += marker.length
      continue
    }
    const unit = units[at]
...
function findClose(tokens: Token[], from: number, marker: string): number {
  ...
  for (let i = from; i < tokens.length; i += 1) {
    ...
    if (token.t !== "delim" || token.marker !== marker) continue
    if (token.opens) {
      nested += 1
      continue
    }
    if (!token.closes) continue
