...
import { fitWheelSensitivity, wheelDeltaForZoomRatio } from '../../harness/driver-geometry'
...
describe('zoomAt', () => {
  ...
  it('is the exponential model the driver fits its sweep against', () => {
    // The driver probes once, fits a sensitivity, then aims every subsequent frame with
    // wheelDeltaForZoomRatio. If the arm's mapping is not exponential in deltaY the fit is
    // wrong everywhere except at the probe, and the sweep silently tracks off schedule.
    const start = { x: 0, y: 0, zoom: 0.4 }
    const probed = zoomAt(start, -120, 0, 0)
    const sensitivity = fitWheelSensitivity(-120, start.zoom, probed.zoom)

    const target = 0.9
    const delta = wheelDeltaForZoomRatio(start.zoom, target, sensitivity)
    expect(zoomAt(start, delta, 0, 0).zoom).toBeCloseTo(target, 9)
  })
})

describe('clampZoom', () => {
  it('holds the shipped desktop limits', () => {
