...
public sealed class ReviewRepository : IReviewRepository
{
    ...
    public async Task<long> RestoreAsync(SqliteConnection conn, SqliteTransaction tx, FlashcardReviewLog log, CancellationToken cancellationToken)
    {
        if (log.Id != FlashcardReviewLog.Unassigned)
        {
            ...
            cmd.CommandText = """
                INSERT INTO FlashcardReviews
                    (Id, CardId, DeckId, SessionId, Grade, ReviewedAt, ElapsedDays, ScheduledDays, StabilityAfter, DifficultyAfter, StateBefore, StateAfter, Origin)
                VALUES ($id, $card, $deck, $session, $grade, $at, $elapsed, $scheduled, $stab, $diff, $before, $state, $origin)
