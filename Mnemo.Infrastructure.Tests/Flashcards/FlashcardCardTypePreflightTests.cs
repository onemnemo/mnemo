...
public sealed class FlashcardCardTypePreflightTests
{
    ...
    [Fact]
    public async Task It_counts_a_live_card_for_a_layout_that_is_already_unlisted()
    {
        await using var h = await OpenAsync();
        await SaveVocabularyAsync(h, "Haus", "house", "Das Haus ist alt.");
        var legacy = WithoutProduction(await VocabularyAsync(h));
        await h.Store.WriteAsync((conn, tx, ct) => h.CardTypes.UpsertAsync(conn, tx, legacy, ct));

        var preflight = await h.FactService.PreviewCardTypeSaveAsync(legacy with { Name = "Words" });
